...
int main(int argc, char *argv[])
{
...

    {
        ...
        auto mesh = std::make_shared<mesh::Mesh<U>>(
            mesh::create_rectangle<U>(MPI_COMM_WORLD, {{{0.0, 0.0}, {2.0, 2.0}}},
            ...
            [](auto x)
            {
                ...
                for (std::size_t p = 0; p < x.extent(1); ++p)
                {
                    ...
                    if ((std::abs(x0) < eps or std::abs(x0 - 2) < eps) ||
                        (std::abs(x1) < eps or std::abs(x1 - 2) < eps))
                        marker[p] = true;
                }
                return marker;
            });
        ...
        f->interpolate(
            [](auto x) -> std::pair<std::vector<T>, std::vector<std::size_t>>
            {
                ...
                for (std::size_t p = 0; p < x.extent(1); ++p)
                {
                    auto dx = (x(0, p) - cx) * (x(0, p) - cx);
                    auto dy = (x(1, p) - cy) * (x(1, p) - cy);
                    f.push_back(1000000 * sin(10 * sqrt(dx + dy)));
                }
        ...
        io::VTKFile filef(MPI_COMM_WORLD, "f.pvd", "w");
        filef.write<T>({*f}, 0);
    }
